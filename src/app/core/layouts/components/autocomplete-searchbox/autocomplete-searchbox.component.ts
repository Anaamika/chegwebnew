...
@Component({
  selector: 'app-autocomplete-searchbox',
...
export class AutocompleteSearchboxComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.searchService.getPopularTags().subscribe(res => {
      if (res.length > 0) {
  ...
  public searchForCompare() {
    if (this.searchQuery != '') {
      ...
      this.onVisible(data, 'keyword', 'Click','search')
  ...
  public serachProducts(data) {
    this.onVisible(data.keyword, 'keyword', 'Click','search')
  ...
  gotoSearch(obj){
    if (utilities.ifObjInArray(this.searchHistory, 'keyword', obj.keyword)) {
      let index = this.searchHistory.findIndex(el => el['keyword'] == obj.keyword);
      this.searchHistory.splice(index, 1);
    }
    obj['type'] = 'Popular Search';
    ...
    this.storageService.setLocalStorage('searchHistory', this.searchHistory);
    this.searchQuery = '';
    this.router.navigate(["product/" + encodeURIComponent(obj.keyword)]);

    this.onVisible(obj.keyword, 'keyword', 'Click','search')
  ...
  saveCategoryInfo(data) {
    this.onVisible(data.keyword, 'keyword', 'Click','category');
    let obj = {};
  ...
  saveBestSellerInfo(data) {
    this.onVisible(data.productName, 'keyword', 'Click','suggestion')
    let obj = {};
  ...
  saveStoreInfo(data) {
    this.onVisible(data.keyword, 'keyword', 'Click','store')
    let obj = {};
  ...
  getOfferInfo($event, data) {
    this.onVisible(data.keyword, 'keyword', 'Click','suggestion')
    $event.stopPropagation();
    $event.preventDefault();
  ...
  onVisible(data, id, eventType,idtype) {
    ...
    arrayObj.page_name = 'search_landing_page';
    arrayObj.section = 'Search';
    arrayObj.id = id;
    arrayObj.idType = idtype;
    arrayObj.name = data;
